...
//     if (game_board.getObject(aX, aY) == '^' || game_board.getObject(aX, aY) == 'v' ||
...
int changeSetting(int &rows, int &columns, int &zombies)
{
    ...
    do
    {
        ...
        { cout << "Invalid input. Row number is too big! Maximum row number is 30. " << endl; }
    ...
    do
    {
        ...
        { cout << "Ivalid input. Column number is too big! Maximum column number is 30. " << endl; }
    ...
    do
    {
        ...
        {
            cout << "Invalid input. Zombie number should be in range of 1 to 9!" << endl;
        }
...
void playGame(int &rows, int &columns, int &zombies)
{
    ...
    // cout << "Alien has landed at position (" << alien.getX() << ", " << alien.getY() << ")" << endl;
    // cout << "Alien      : Life " << alien.getAlienLife() << endl;
    // cout << "Zombie: " << endl;
    ...
    {
        cout << "Alien      : Life " << alien.getAlienLife() << ", Attack: " << alien.getAlienAttack() << endl;
        cout << "Zombie     : Life " << zombie.getZombieLife() << ", Attack: " << zombie.getZombieAttack() << ", Range: " << zombie.getZombieRange() << endl;
        cout << "Command => ";
        cin >> commseq;
        ...
        else if (commseq != "up" || commseq != "down" || 
        commseq != "left" || commseq != "right" || 
        commseq != "help" || commseq != "save" || 
        commseq != "load" || commseq != "quit") 
        {command = 'I';} // invalid command
...
void randomZombieMove(string **gameboard, int zcount, int row, int column, int **Zombiearr)
{
    for (int i = 0; i < zcount; i++)
    {
        ...
        {
            if (prev_row < row - 1) // check if the zombie is not already at the bottom row
            {
                Zombiearr[i][0] = prev_row + 1;
                gameboard[prev_row][prev_col] = ".";
                gameboard[prev_row + 1][prev_col] = to_string(i + 1);
        ...
        {
            if (prev_col > 0) // check if the zombie is not already at the leftmost column
            {
        ...
        {
            if (prev_col < column - 1) // check if the zombie is not already at the rightmost column
            {
                Zombiearr[i][1] = prev_col + 1;
                gameboard[prev_row][prev_col] = ".";
                gameboard[prev_row][prev_col + 1] = to_string(i + 1);
